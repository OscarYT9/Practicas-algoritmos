...
def ins_sort(v):
    for i in range(1,len(v)): #Buscar traducir el pseudocodigo a python, nos dice que el algoritmo empieza en la posicion 2 del algoritmo y en python tiene que ir de la posicion 1 a la n, ya que la primera posición pyhton es el 0
        x = v[i]
        j = i-1
        while j>=0 and v[j]>x:
            v[j+1] = v[j]
